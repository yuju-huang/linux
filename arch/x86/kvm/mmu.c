 ...
 */
static void mmu_spte_clear_no_track(struct kvm *kvm, u64 *sptep)
{
	__update_clear_spte_fast(sptep, 0ull);
    delete_dsag_node(kvm, sptep);
}
...
static void pte_list_remove(struct kvm *kvm,struct kvm_rmap_head *rmap_head, u64 *sptep)
{
					       ...
					       unsigned long data))
{
	...
	for (i = 0; i < KVM_ADDRESS_SPACE_NUM; i++) {
		...
		kvm_for_each_memslot(memslot, slots) {
            ...
            dsag_printk(KERN_DEBUG, "%s: gfn_start=0x%llx, gfn_end=0x%llx\n", __func__, gfn_start, gfn_end);
...
{
    dsag_printk(KERN_DEBUG, "%s, start=0x%lx, end=0x%lx\n", __func__, start, end);
                            ...
                            u64 *parent_pte)
{
	...
	mmu_spte_clear_no_track(kvm, parent_pte);
}
#else
			     ...
			     u64 *spte)
{
	...
	if (is_mmio_spte(pte))
		mmu_spte_clear_no_track(kvm, spte);
		       	...
		       	bool speculative, bool host_writable)
{
	...
	dsag_printk(KERN_DEBUG,"%s: sptep 0x%lx spte 0x%llx write_fault %d gfn 0x%llx\n", __func__, (uintptr_t)sptep,
		 *sptep, write_fault, gfn);
	...
	if (is_shadow_present_pte(*sptep)) {
		...
		if (level > PT_PAGE_TABLE_LEVEL &&
            ...
            dsag_printk(KERN_DEBUG, "%s-2, pfn=0x%llx, sptep=0x%lx, *sptep=0x%llx, spte_to_pfn=0x%lx\n", __func__, pfn, (uintptr_t)sptep, *sptep, spte_to_pfn);
    ...
    dsag_printk(KERN_DEBUG, "%s-8\n", __func__);
	if (unlikely(is_mmio_spte(*sptep)))
		ret = RET_PF_EMULATE;

	pgprintk("%s: setting spte %llx\n", __func__, *sptep);
	dsag_printk(KERN_DEBUG,"%s: setting spte 0x%llx\n", __func__, *sptep);
...
#if 0
        dsag_printk(KERN_DEBUG, "%s; sptep=0x%lx, spte=0x%llx, iter.level=%d, level=%d\n", __func__, (uintptr_t)iterator.sptep, *iterator.sptep, iterator.level, level);
			  ...
			  bool prefault)
{
    ...
    dsag_printk(KERN_DEBUG, "tdp_page_fault1, vpuc=0x%lx, gpa=0x%lx, gfn=0x%llx, error_code=0x%x\n", (uintptr_t)vcpu, gpa, gfn, error_code);

	MMU_WARN_ON(!VALID_PAGE(vcpu->arch.mmu->root_hpa));

	if (page_fault_handle_page_track(vcpu, error_code, gfn)) {
        dsag_printk(KERN_DEBUG, "tdp_page_fault return 1\n");
        return RET_PF_EMULATE;
    }

	r = mmu_topup_memory_caches(vcpu);
	if (r) {
        dsag_printk(KERN_DEBUG, "tdp_page_fault return 2, r=%d\n", r);
		return r;
    }
	...
	if (fast_page_fault(vcpu, gpa, level, error_code)) {
        dsag_printk(KERN_DEBUG, "tdp_page_fault return 3\n");
		return RET_PF_RETRY;
    }
#endif
	...
	if (try_async_pf(vcpu, prefault, gfn, gpa, &pfn, write, &map_writable)) {
        dsag_printk(KERN_DEBUG, "tdp_page_fault return 4\n");
		return RET_PF_RETRY;
    }

	if (handle_abnormal_pfn(vcpu, 0, gfn, pfn, ACC_ALL, &r)) {
        dsag_printk(KERN_DEBUG, "tdp_page_fault return 5, r=%d\n", r);
		return r;
    }

	spin_lock(&vcpu->kvm->mmu_lock);
	if (mmu_notifier_retry(vcpu->kvm, mmu_seq)) {
        dsag_printk(KERN_DEBUG, "tdp_page_fault return 6\n");
		goto out_unlock;
    }
	if (make_mmu_pages_available(vcpu) < 0) {
        dsag_printk(KERN_DEBUG, "tdp_page_fault return 7\n");
		goto out_unlock;
    }
	if (likely(!force_pt_level))
		transparent_hugepage_adjust(vcpu, &gfn, &pfn, &level);
    ...
    dsag_printk(KERN_DEBUG, "tdp_page_fault2, gpa=0x%lx, gfn=0x%llx, pfn=0x%llx, error_code=0x%x\n", gpa, gfn, pfn, error_code);

	r = __direct_map(vcpu, write, map_writable, level, gfn, pfn, prefault);
	spin_unlock(&vcpu->kvm->mmu_lock);

    dsag_printk(KERN_DEBUG, "tdp_page_fault return 8, r=%d\n", r);
	return r;
			   ...
			   unsigned access, int *nr_present)
{
	if (unlikely(is_mmio_spte(*sptep))) {
		if (gfn != get_mmio_spte_gfn(*sptep)) {
			mmu_spte_clear_no_track(vcpu->kvm, sptep);
			return true;
		}
